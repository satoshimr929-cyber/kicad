...
- **編集**
  ...
  - **フットプリント編集**: Footprint 欄の「フットプリントを編集…」から pcbnew 風の
    エディタを起動。公式ライブラリ全量（デプロイ時に同梱）またはローカルの
    `.kicad_mod` を読み込み、パッド（SMD/スルーホール・ドリル・番号）・シルク／Fab／
    courtyard の図形・テキストを描画。タップ選択 → ドラッグ移動（0.05mm スナップ）、
    番号・位置・サイズ・回転・ドリル径の数値編集、削除、Undo/Redo、名前変更に対応し、
    結果を **`.kicad_mod` としてダウンロード**（PC の KiCad でそのまま利用可能）
